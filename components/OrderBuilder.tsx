...
const OrderBuilder: React.FC<OrderBuilderProps> = ({ onTotalUnitsChange }) => {
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    const baseOrder = {
      ...
      deliveryFee: settings.defaultDeliveryFee || 0,
      totalEstimated: subtotal + (settings.defaultDeliveryFee || 0),
      status: 'novo' as const,
      statusHistory: [{ status: 'novo', changedAt: now, note: 'Pedido criado na landing' }],
      internalNotes: '',
      source: 'landing_page',
      whatsappMessage: '',
      createdAt: now,
