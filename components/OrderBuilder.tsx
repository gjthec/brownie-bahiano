...
import { buildWhatsAppMessage, ensureSeedData, getFlavors, getSettings, normalizePhone, persistOrder } from '../lib/storage';
...
const OrderBuilder: React.FC<OrderBuilderProps> = ({ onTotalUnitsChange }) => {
  ...
  useEffect(() => {
    ensureSeedData();
    setFlavors(getFlavors().filter((f) => f.active));
  ...
  const handleSubmit = (event: React.FormEvent) => {
    ...
    try {
      ...
      setFeedback({
        type: 'success',
        message: `Pedido ${order.orderNumber} salvo com sucesso! Abrindo WhatsApp...`,
      });
      setFormData({ nome: '', telefone: '', tipoDeNegocio: '', bairro: '', formaEntrega: 'Entrega', observacoes: '' });
      setQtyByFlavor({});
    } catch {
      setFeedback({ type: 'error', message: 'Não foi possível salvar o pedido. Tente novamente.' });
    } finally {
  ...
  return (
    <Section id="monte-seu-pedido" darker>
      <div className="text-center mb-12">
        ...
        <p className="text-stone-400 max-w-2xl mx-auto">Escolha os sabores, informe as quantidades e envie tudo pronto para o WhatsApp.</p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="grid lg:grid-cols-3 gap-8">
        ...
        <aside className="bg-brand-primary border border-white/10 rounded-xl p-5 h-fit lg:sticky lg:top-24">
          ...
          <div className="mt-4 bg-brand-gold/10 border border-brand-gold/30 rounded-lg p-3 text-xs text-stone-200 flex gap-2">
            ...
            Pedido mínimo atual: {settings.minimumOrderUnits} unidades. O pedido será registrado no sistema e confirmado via WhatsApp.
          </div>
